...
impl DocGenerator for CppAiGenerator {
    async fn generate(&self, ctx: &GeneratorCtx) -> Result<GeneratedDocs> {
        let cfg = ctx
            ...
            .extra
            .as_ref()
            .and_then(|e| e.get("cppAiConfig"))
            .cloned();
        let Some(cfg) = cfg else {
            anyhow::bail!("cppAiConfig missing for SDK {}", ctx.sdk.id);
        };
        let spec_path = cfg
            .get("specPath")
            .and_then(|v| v.as_str())
        ...
        let models_path = cfg
            .get("modelsPath")
            .and_then(|v| v.as_str())
            .unwrap_or("client/include/FastCommentsClient/model/");
        ...
        let parser = CppParser::new(ctx.repo_path.clone(), models_path);
        ...
        let llm = LlmClient::new(&cache_dir, &model, "cpp")?;
        ...
        for api_file in &api_files {
            ...
            for mut m in methods {
                ...
                if let Some(info) = info {
                    ...
                    // OpenAPI description overrides the parser-extracted
                    // description when present. Mirrors line 69.
        ...
        for (idx, method) in all_methods.into_iter().enumerate() {
            ...
            tasks.push(tokio::spawn(async move {
                ...
                let prompt = prompts::cpp_prompt(&method);
...
fn build_method_section(
    ...
    if !code_example.is_empty() {
        ...
        lines.push(format!(
            "[inline-code-attrs-start title = '{title}'; type = 'cpp'; isFunctional = false; inline-code-attrs-end]"
        ));
...
fn type_github_url(file_path: &str, sdk: &crate::config::SdkConfig, models_path_rel: &str) -> String {
    // Mirrors cpp-ai-generator.js:240 — Node concatenates `modelsPath`
    // with the file path that's already repo-root-relative, producing
    // a doubled path. We preserve that bug for byte parity.
    let base = sdk.repo.trim_end_matches(".git").trim_end_matches('/');
    format!("{base}/blob/{branch}/{models_path_rel}{file_path}", branch = sdk.branch)
}
