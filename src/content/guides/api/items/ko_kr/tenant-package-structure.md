...
A tenant may have multiple tenant packages and have the ability to change the package themselves from the [청구 정보 페이지.](https://fastcomments.com/auth/my-account/billing-info)
...
[inline-code-attrs-start title = 'TenantPackage 구조'; type = 'typescript'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
export interface TenantPackage {
    ...
    flexSSOUserCostCents?: null // 일반 SSO 사용자(관리자/모더레이터 권한 없음)당 비용
    ...
    flexDomainUnit?: null
    flexSSOAdminCostCents?: null // 관리자 권한이 있는 SSO 사용자당 비용 (isAccountOwner 또는 isAdminAdmin 플래그)
    flexSSOAdminUnit?: null
    flexSSOModeratorCostCents?: null // 모더레이터 권한이 있는 SSO 사용자당 비용 (isCommentModeratorAdmin 플래그)
