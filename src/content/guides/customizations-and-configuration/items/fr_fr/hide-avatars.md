---
[related-parameter-start name = 'hideAvatars'; type = 'boolean'; related-parameter-end]

Les avatars peuvent être complètement supprimés du widget de commentaires, même si les utilisateurs ont défini leur propre avatar. 

[code-example-start config = {hideAvatars: true}; linesToHighlight = [6]; title = 'Hiding Avatars'; code-example-end]

Cela peut être personnalisé sans code, sur la page de personnalisation du widget :

[app-screenshot-start url='/auth/my-account/customize-widget/new'; selector = '.hide-avatars'; alt='Option Masquer les avatars sur la page de personnalisation du widget, qui supprime tous les avatars du widget de commentaires'; title='Masquer les avatars' app-screenshot-end]

---