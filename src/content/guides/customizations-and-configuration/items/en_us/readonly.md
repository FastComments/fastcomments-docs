...
Commenting can be locked so no new comments or votes can be posted by setting the readonly flag to true.

Comments will also be unable to be edited or deleted.

[code-example-start config = {readonly: true}; linesToHighlight = [6]; title = 'Making the Comment Thread Read-only'; code-example-end]

This can be customized without code, on the widget customization page, for an entire domain, or page:

[app-screenshot-start url='/auth/my-account/customize-widget/new'; selector = '.prevent-new-replies'; title='Making the Comment Thread Read-only' app-screenshot-end]
