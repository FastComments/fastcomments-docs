...
[app-screenshot-start url='/auth/my-account/customize-widget/new'; clickSelectors = ['.enable-view-counts']; selector = '.enable-view-counts'; alt='Stranica za prilagođavanje widgeta sa označenim poljem za prikaz broja pregleda, tako da svaki komentar prikazuje ikonu oka i broj'; title='Omogućavanje broja pregleda komentara' app-screenshot-end]

Pratimo ID korisnika* koji je pogledao komentar tokom jedne nedelje, tako da ako ponovo pogledate komentar u tom periodu, broj se ne povećava. Ako pogledate komentar ponovo nakon što nedelja prođe, broj će se ponovo povećati.

- *Napomena: ili anonimni ID sesije, ili IP korisnika kao heširana vrednost.