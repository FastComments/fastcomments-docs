[inline-code-attrs-start title = 'CSS по умолчанию виджета комментариев'; type = 'css'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
body { margin: 0; padding: 0; }
    ...
    .pagination .load-all { padding-left: 25px; border-left: 2px solid #555; } /* важно, чтобы граница была на правом элементе, так как она не всегда отображается */
    ...
    .comment-reply .comment-reply-top-bar { position: relative; min-height: 25px; margin: 0 26px 15px 26px; line-height: 25px; } /* относительное позиционирование для таких элементов, как список уведомлений */
    ...
    .comment .commenter-name .label { font-size: 10px; text-transform: uppercase; font-weight: 500; color: #666666; } /* общий стиль меток (метка непроверенного, метка администратора, пользовательские метки и т.д.) */
    ...
    .comment .prompt { position: absolute; top: -2px; left: 0; width: 100%; height: 100%; padding: 10px; box-sizing: border-box; background: rgba(255, 255, 255, 0.9); text-align: center } /* -2px чтобы гарантировать перекрытие стрелки */
    ...
    @media(max-width: 500px) { .comment > .inner > .top-right { top: -10px; } } /* если значение top слишком мало, элемент окажется поверх подсветки при действиях администратора */
