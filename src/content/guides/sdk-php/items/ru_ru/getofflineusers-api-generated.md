...
| urlId | string | query | Да | Идентификатор URL страницы (очищенный на сервере). |
| afterName | string | query | Нет | Курсор: передайте nextAfterName из предыдущего ответа. |
| afterUserId | string | query | Нет | Разделитель курсора: передайте nextAfterUserId из предыдущего ответа. Требуется, когда afterName задан, чтобы привязки по имени не удаляли записи. |
...
$options = [
    ...
    'after_user_id' => 'after_user_id_example', // string | Разделитель курсора: передайте nextAfterUserId из предыдущего ответа. Требуется, когда afterName задан, чтобы привязки по имени не удаляли записи.
];
