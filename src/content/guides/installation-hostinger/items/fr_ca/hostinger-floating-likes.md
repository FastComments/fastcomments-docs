FastComments prend également en charge le widget Page Reacts (aussi appelé bouton « J’aime » flottant) pour Hostinger.

Vous pouvez le voir en action en bas à droite de cette page !

### Remarque !

Ces instructions s'appliquent au Site Builder de Hostinger. Si vous utilisez Hostinger *WordPress*, copiez simplement le code ci‑dessous et ajoutez‑le à votre site WordPress en utilisant [WPCode](https://wordpress.org/plugins/insert-headers-and-footers/), qui est un plugin gratuit et facile pour ajouter de petits extraits de code à votre site.

1. Tout d'abord, récupérez le code :

[inline-code-attrs-start title = 'Code des mentions J’aime flottantes Hostinger'; type = 'bash'; useDemoTenant = false; isFunctional = false; type = 'html';  inline-code-attrs-end]
[inline-code-start]
...
6. Vous ne verrez pas le widget en mode de prévisualisation, mais il apparaîtra sur la version publiée du site.