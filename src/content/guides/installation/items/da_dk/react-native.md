...
// ...
  ...
  const tenantId = 'demo'; // Dit tenant id. Kan hentes fra https://fastcomments.com/auth/my-account/api-secret
  const pageId = 'native-test'; // ID eller URL for kommentartråden i din app.
  const config = {
    tenantId: tenantId,
    urlId: pageId
  };
...
Hvis du er i EU, vil du gerne sætte `region`-parameteren:
