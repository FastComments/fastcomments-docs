Теперь, когда мы в редакторе шаблонов, нужно решить, где мы хотим отображать комментарии или живой чат.

В этом примере мы добавим его прямо под видео. Наведите курсор на элемент, в конец которого нужно добавить виджет, и нажмите `ADD ELEMENT`:
...
Теперь откроем редактор кода, куда вставим наш код.

На следующем шаге ClickFunnels может сбить с толку.

Важно *НЕ* выбирать `Code`, когда вы наводите курсор на новый элемент. Вместо этого выберите `SETTINGS`:
...
Теперь справа можно нажать `Open Code Editor`:
...
Откроется большое окно. Здесь мы сможем вставить наш код. Скопируйте следующий фрагмент (используйте кнопку копирования в правом верхнем углу):

[inline-code-attrs-start title = 'Фрагмент кода потокового чата ClickFunnels'; type = 'html'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
...
<script>
    (function fcLoad() {
        function tryLoad() {
            // некоторые провайдеры меняют фрагмент кода на async
...
Этот фрагмент кода предназначен для нашего продукта Streaming Chat, который отлично подходит для видео. Если вам нужен фрагмент кода виджета Live Commenting, который лучше подходит для обычных страниц или блогов, он находится в конце этого руководства.
...
Теперь осталось только закрыть окно:
...
Теперь вы можете предварительно просмотреть изменения! Не стесняйтесь перемещать виджет и выбирать наилучшее расположение.
...
Готово! Не забудьте проверить на мобильных устройствах!
