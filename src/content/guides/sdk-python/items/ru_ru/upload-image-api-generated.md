...
| sizePreset | string | query | Нет | Набор размеров: "Default" (1000x1000px) или "CrossPlatform" (создает размеры для популярных устройств) |
...
with client.ApiClient(configuration) as api_client:
    ...
    file = None # bytearray | 
    size_preset = client.SizePreset() # SizePreset | Набор размеров: \"Default\" (1000x1000px) или \"CrossPlatform\" (создает размеры для популярных устройств) (необязательно)
    url_id = 'url_id_example' # str | Идентификатор страницы, с которой выполняется загрузка, для настройки (необязательно)
