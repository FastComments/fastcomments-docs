...
Instaliraj izravno s oznakom izdanja (preporučeno, potpuno reproducibilno):
...
Svako označeno [GitHub izdanje](https://github.com/FastComments/fastcomments-python/releases) također ima priloženi wheel ako radije izravno instaliraš binarni artefakt.
...
Za API klijent postoje tri klase, `DefaultApi`, `PublicApi` i `ModerationApi`. `DefaultApi` sadrži metode koje zahtijevaju tvoj API ključ, a `PublicApi` sadrži metode koje se mogu izravno pozvati iz preglednika/mobilnog uređaja itd. bez autentifikacije. `ModerationApi` pruža opsežan skup live i brzih moderacijskih API-ja. Svaka metoda `ModerationApi` prihvaća parametar `sso` i može se autentificirati putem SSO-a ili sesijskog kolačića FastComments.com.