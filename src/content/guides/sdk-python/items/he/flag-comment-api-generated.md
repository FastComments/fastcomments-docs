...
[inline-code-attrs-start title = 'דוגמת flag_comment'; type = 'python'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
import client
...
# הגדרת ה‑host היא אופציונלית והערך ברירת המחדל הוא https://fastcomments.com
# ראה configuration.py לקבלת רשימה של כל פרמטרי הקונפיגורציה הנתמכים.
# על הלקוח להגדיר את פרמטרי האימות וההרשאה
# בהתאם למדיניות האבטחה של שרת ה‑API.
# דוגמאות לכל סוג אימות ניתנות למטה, השתמשו בדוגמה המתאימה למקרה שלכם.

# הגדרת הרשאת מפתח API: api_key
configuration.api_key['api_key'] = os.environ["API_KEY"]

# בטל את ההערה שלמטה כדי להגדיר קידומת (למשל Bearer) למפתח ה‑API, אם נדרש
