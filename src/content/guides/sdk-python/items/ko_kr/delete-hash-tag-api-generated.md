...
[inline-code-attrs-start title = 'delete_hash_tag 예제'; type = 'python'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
import client
...
# 아래에 각 인증 방식에 대한 예제가 제공되어 있으니, 해당
# 인증 사용 사례에 맞는 예제를 사용하세요.

# API 키 인증 구성: api_key
configuration.api_key['api_key'] = os.environ["API_KEY"]

# 필요 시 API 키에 대한 접두사(예: Bearer)를 설정하려면 아래의 주석을 해제하세요
# configuration.api_key_prefix['api_key'] = 'Bearer'

# API 클라이언트 인스턴스를 사용한 컨텍스트 진입
with client.ApiClient(configuration) as api_client:
    # API 클래스의 인스턴스 생성
    api_instance = client.DefaultApi(api_client)
    tag = 'tag_example' # str | 
    tenant_id = 'tenant_id_example' # str |  (선택 사항)
    delete_hash_tag_request = client.DeleteHashTagRequest() # DeleteHashTagRequest |  (선택 사항)
