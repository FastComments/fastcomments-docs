...
戻り値: [`PutSSOUserAPIResponse`](https://github.com/FastComments/fastcomments-python/blob/main/client/models/put_sso_user_api_response.py)

## 例

[inline-code-attrs-start title = 'put_sso_user の例'; type = 'python'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
import client
...
# ホストの定義は任意で、デフォルトは https://fastcomments.com
...
# 各認証方式の例を以下に示します。自分の認証ケースに合う例を使用してください。

# Configure API key authorization: api_key
configuration.api_key['api_key'] = os.environ["API_KEY"]

# 必要な場合は下のコメントアウトを解除して API キーのプレフィックス（例: Bearer）を設定してください
...
with client.ApiClient(configuration) as api_client:
    ...
    update_comments = True # bool |  （オプション）
