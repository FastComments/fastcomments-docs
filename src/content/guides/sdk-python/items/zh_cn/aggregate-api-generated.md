通过对文档进行分组（如果提供了 groupBy）并应用多个运算来聚合文档。支持不同的运算（例如 sum、countDistinct、avg 等）。
...
返回: [`AggregateResponse`](https://github.com/FastComments/fastcomments-python/blob/main/client/models/aggregate_response.py)

## 示例

[inline-code-attrs-start title = 'aggregate 示例'; type = 'python'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
import client
...
# 客户端必须根据 API 服务器的安全策略来配置身份验证和授权参数。
