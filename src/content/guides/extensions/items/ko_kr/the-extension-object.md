...
<!-- 이 것을 업데이트하려면 comment-ui-core도 업데이트해야 합니다. -->

[inline-code-attrs-start title = '확장 객체 JSDoc'; type = 'javascript'; inline-code-attrs-end]
[inline-code-start]
/**
 * FastCommentsUI 확장 객체입니다. 특정 컴포넌트를 지연 로드하는 데 사용됩니다. 예를 들어, 리뷰 시스템은 모든 고객이 사용하지 않으므로
 * 우리가 원할 때만 해당 확장을 로드합니다.
 *
 ...
 * @property {string} [css]
 * @property {Object} config - FastComments 설정 객체.
 ...
 * @property {FastCommentsUIExtensionPrepareCommentForSavingCallback} [prepareCommentForSaving] - 게시될 댓글과 함께 호출됩니다. false를 반환하면 제출을 취소합니다(예: 연결된 설문조사가 완전하지 않은 경우).
