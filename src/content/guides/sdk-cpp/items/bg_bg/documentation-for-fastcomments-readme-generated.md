...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Агрегира документи чрез групиране (ако се предостави `groupBy`) и прилагане на множество операции. Поддържат се различни операции (например sum, countDistinct, avg и др.). |
...
*DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts |  изисква tenantId afterId |
...
*PublicApi* | [**getCommentsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getCommentsPublic) | **GET** /comments/{tenantId} |  изисква tenantId urlId |
*PublicApi* | [**getEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getEventLog) | **GET** /event-log/{tenantId} |  изисква tenantId urlId userIdWS |
*PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} |  изисква tenantId afterId |
...
*PublicApi* | [**getGlobalEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getGlobalEventLog) | **GET** /event-log/global/{tenantId} |  изисква tenantId urlId userIdWS |
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Предишни коментатори на страницата, които **НЕ** са онлайн в момента. Подреждане по displayName. Използвайте след като изчерпате /users/online, за да визуализирате раздел „Members“. Курсорна пагинация по commenterName: сървърът обходва частичния {tenantId, urlId, commenterName} индекс след afterName чрез $gt, без разход за $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Профили на потребители, които в момента са онлайн на страницата: хора, чието websocket сесия е абонирана за страницата в момента. Връща anonCount + totalCount (абонати за стаята, включително анонимни зрители, които не се изброяват). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Списък с страниците за даден наемател. Използва се от настолен клиент FChat за попълване на списъка със стаи. Изисква `enableFChat` да бъде true в резолвнатата потребителска конфигурация за всяка страница. Страници, които изискват SSO, се филтрират спрямо груповия достъп на заявилия потребител. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Масивна информация за потребителите за даден наемател. При зададени userIds се връща информация за показване от User / SSOUser. Използва се от widget-а за коментари, за да обогати потребителите, които току‑що се появяват чрез събитие за присъствие. Без контекст на страницата: поверителността се прилага еднозначно (частни профили се маскират). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Качване и преоразмеряване на изображение |
...
## Документация за модели
 ...
 - [CreateAPIUserSubscriptionData](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/