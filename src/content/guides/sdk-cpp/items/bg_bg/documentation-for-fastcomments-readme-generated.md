...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Агрегира документи чрез групиране (ако е предоставено groupBy) и прилагане на множество операции. Поддържат се различни операции (например sum, countDistinct, avg и т.н.). |
...
*DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts |  изисква tenantId след afterId |
...
*PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} |  изисква tenantId след afterId |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Предишни коментиращи на страницата, които НЕ са в момента онлайн. Сортирани по displayName. Използвайте това след като изчерпате /users/online, за да покажете "Members" секция. Курсорно пагиниране по commenterName: сървърът обхожда индекса {tenantId, urlId, commenterName} от afterName напред чрез $gt, без $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | В момента онлайн посетители на страницата: хора, чиито websocket сесии са абонирани за страницата в този момент. Връща anonCount + totalCount (абонати за стаята, включително анонимни зрители, които не изброяваме). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Изброява страниците за tenant. Използва се от FChat настолния клиент за попълване на списъка със стаи. Изисква `enableFChat` да е true в крайната потребителска конфигурация за всяка страница. Страници, които изискват SSO се филтрират спрямо достъпа на групата на заявителя. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Масова информация за потребители в tenant. При дадени userIds връща информация за показване от User / SSOUser. Използва се от widget-а за коментари, за да обогати потребителите, които току-що са се появили чрез събитие за присъствие. Без контекст на страница: поверителността е осигурена еднакво (частните профили са маскирани). |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | Активирайте или деактивирайте известия за конкретен коментар. |
*PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | Активирайте или деактивирайте известия за страница. Когато потребителите са абонирани за страница, създават се известия за нови коренови коментари, както и |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Качете и оразмерете изображение |
...
## Документация за Модели
 ...
 - [DeleteSSOUserAPIResponse](https://github.com/FastComments/fastcomments-c