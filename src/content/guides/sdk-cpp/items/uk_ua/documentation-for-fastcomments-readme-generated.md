...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Агрегує документи шляхом їх групування (якщо groupBy вказаний) і застосування різних операцій. Підтримуються різні операції (наприклад, sum, countDistinct, avg тощо). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Колишні коментатори сторінки, які ЗАРАЗ не є онлайн. Сортування за displayName. Використовуйте це після того, як вичерпали список /users/online для відображення секції "Учасники". Курсорна пагінація за commenterName: сервер перебирає частковий індекс {tenantId, urlId, commenterName} починаючи з afterName через $gt, без витрат $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Поточні онлайн-глядачі сторінки: люди, чия websocket-сесія зараз підписана на цю сторінку. Повертає anonCount + totalCount (всі підписники кімнати, включаючи анонімних, які не перераховані). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Список сторінок для tenant. Використовується десктоп-клієнтом FChat для підвантаження списку кімнат. Потрібно, щоб параметр `enableFChat` був true у кастомній конфігурації для кожної сторінки. Сторінки з вимогою SSO фільтруються відповідно до групових доступів користувача-запитувача. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Масове отримання інформації про користувачів для tenant. За userIds повертається відображувана інформація з User / SSOUser. Використовується віджетом коментарів для збагачення користувачів, які тільки з'явилися через події присутності. Немає контексту сторінки: приватність дотримується однаково (приватні профілі маскуються). |
