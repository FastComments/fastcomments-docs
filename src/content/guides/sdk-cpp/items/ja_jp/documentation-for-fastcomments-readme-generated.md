...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | ドキュメントを集約し、それらをグループ化（groupByが指定されていれば）し、複数の演算を適用します。さまざまな演算（例：sum、countDistinct、avgなど）に対応しています。 |
...
*DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts |  必須: tenantId afterId |
...
*PublicApi* | [**getCommentsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getCommentsPublic) | **GET** /comments/{tenantId} |  必須: tenantId urlId |
*PublicApi* | [**getEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getEventLog) | **GET** /event-log/{tenantId} |  必須: tenantId urlId userIdWS |
*PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} |  必須: tenantId afterId |
...
*PublicApi* | [**getGlobalEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getGlobalEventLog) | **GET** /event-log/global/{tenantId} |  必須: tenantId urlId userIdWS |
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | ページ上で過去にコメントしたが現在オンラインでないユーザー。displayNameでソートされます。/users/onlineを使い切った後に「メンバー」セクションのレンダリングに利用してください。commenterNameでのカーソルページネーション: サーバーは部分的な{tenantId, urlId, commenterName}インデックスをafterNameから$gtで前方走査し、$skipコストはありません。 |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | ページの現在オンライン中の閲覧者: 今まさにwebsocketセッションがページにサブスクライブしている人々を返します。匿名閲覧者の列挙を含まず、room全体のanonCount + totalCountを返します。 |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | テナントのページ一覧。FChatデスクトップクライアントがルームリストを構築するのに用います。各ページのカスタム設定のenableFChatがtrueであることが必要です。SSOが必要なページは、リクエスト中のユーザーのグループアクセス権でフィルタリングされます。 |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | テナント用のユーザー情報を一括取得します。userIdsを指定すると、User / SSOUserから表示情報を返します。コメントウィジェットがプレゼンスイベント経由で現れたユーザー情報の拡充に利用されます。ページコンテキスト不要: プライバシーは常に同じ（非公開プロフィールはマスクされる）形で強制されます。 |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | 特定のコメントに対する通知の有効化・無効化 |
*PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | ページ単位の通知の有効化・無効化を行います。ユーザーがページにサブスクライブしている場合、新しいルートコメントに対して通知が作成されます。また、 |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | 画像をアップロードしリサイズします |
