...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Agrupa documentos mediante su agrupación (si se proporciona groupBy) y aplica múltiples operaciones. Se admiten diferentes operaciones (p. ej., sum, countDistinct, avg, etc.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Usuarios que comentaron anteriormente en la página que NO están actualmente en línea. Ordenados por displayName. Use esto después de agotar /users/online para renderizar una sección \"Members\". Paginación por cursor en commenterName: el servidor recorre el índice parcial {tenantId, urlId, commenterName} desde afterName hacia adelante mediante $gt, sin costo de $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Visitantes actualmente en línea de una página: personas cuya sesión de websocket está suscrita a la página en este momento. Devuelve anonCount + totalCount (suscriptores de la sala, incluidos espectadores anónimos que no enumeramos). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Enumera las páginas de un tenant. Utilizado por el cliente de escritorio FChat para rellenar su lista de salas. Requiere que `enableFChat` sea true en la configuración personalizada resuelta de cada página. Las páginas que requieren SSO se filtran según el acceso grupal del usuario solicitante. |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Subir y redimensionar una imagen |
...
## Documentación de los Modelos
 ...
 - [BuildModerationFilterParams](https://github