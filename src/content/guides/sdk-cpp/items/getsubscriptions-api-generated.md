...
api->getSubscriptions(tenantId, userId).then([](pplx::task<std::shared_ptr<GetSubscriptionsAPIResponse>> task){
    try {
        auto resp = task.get();
        auto stored = resp ? resp : std::make_shared<GetSubscriptionsAPIResponse>();
        return stored;
    } catch (...) {
        return std::make_shared<GetSubscriptionsAPIResponse>();
    }
});
[inline-code-end]
