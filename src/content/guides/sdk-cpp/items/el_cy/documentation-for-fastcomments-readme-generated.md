...
Όλα τα URIs είναι σχετικά με το *https://fastcomments.com*
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Συγκεντρώνει έγγραφα ομαδοποιώντας τα (αν δοθεί το groupBy) και εφαρμόζοντας πολλαπλές λειτουργίες. Υποστηρίζονται διάφορες λειτουργίες (π.χ. sum, countDistinct, avg, κλπ.). |
...
*DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts |  απαιτεί tenantId μετά το afterId |
...
*PublicApi* | [**getCommentsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getCommentsPublic) | **GET** /comments/{tenantId} |  απαιτεί tenantId urlId |
*PublicApi* | [**getEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getEventLog) | **GET** /event-log/{tenantId} |  απαιτεί tenantId urlId userIdWS |
*PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} |  απαιτεί tenantId μετά το afterId |
...
*PublicApi* | [**getGlobalEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getGlobalEventLog) | **GET** /event-log/global/{tenantId} |  απαιτεί tenantId urlId userIdWS |
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Παλαιοί σχολιαστές στη σελίδα που δεν είναι ενεργοί αυτή τη στιγμή. Ταξινομημένοι κατά displayName. Χρησιμοποιήστε το αφού εξαντλήσετε το /users/online για να εμφανίσετε ενότητα "Μέλη". Paginάρισμα με cursor στο commenterName: ο διακομιστής περπατά το μερικό ευρετήριο {tenantId, urlId, commenterName} από το afterName μπροστά μέσω $gt, χωρίς κόστος $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Τρέχοντες χρήστες που βλέπουν τη σελίδα: χρήστες που η websocket συνεδρία τους είναι συνδρομημένη στη συγκεκριμένη σελίδα αυτή τη στιγμή. Επιστρέφει anonCount + totalCount (συνδρομητές στο δωμάτιο, συμπεριλαμβανομένων ανώνυμων που δεν απαριθμούμε). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Λίστα σελίδων για μια ενοικίαση. Χρησιμοποιείται από τον FChat desktop client για τη συμπλήρωση της λίστας δωματίων του. Απαιτεί το `enableFChat` να είναι true στη τελική custom ρύθμιση κάθε σελίδας. Οι σελίδες που απαιτούν SSO φιλτράρονται ως προς την πρόσβαση της ομάδας του αιτούντα χρήστη. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Μαζικές πληροφορίες χρηστών για έναν tenant. Δίνοντας userIds, επιστρέφει display info από User / SSOUser. Χρησιμοποιείται από το comment widget για εμπλουτισμό χρηστών που μόλις εμφανίστηκαν μέσω event παρουσίας. Δεν υπάρχει συμφραζόμενο σελίδας: το απόρρητο εφαρμόζεται ομοιόμορφα (τα ιδιωτικά προφίλ καλύπτονται). |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | Ενεργοποιήστε ή απενεργοποιήστε τις ειδοποιήσεις για συγκεκριμένο σχόλιο. |
*PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | Ενεργοποιήστε ή απενεργοποιήστε ειδοποιήσεις για μια σελίδα. Όταν οι χρήστες είναι συνδρομημένοι σε μια σελίδα, δημιουργούνται ειδοποιήσεις για νέα root comments, και επίσης |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Ανεβάστε και προσαρμόστε μέγεθος εικόνας |
...
## Τεκμηρίωση για τα Μοντέλα
 ...
 - [DeleteFeedPostPublicResponse](https://github.com/FastComments/fastcomments-cpp/blob/master