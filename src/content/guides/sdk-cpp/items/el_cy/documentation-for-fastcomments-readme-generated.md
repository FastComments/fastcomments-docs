...
| Κατηγορία | Μέθοδος | Αίτημα HTTP | Περιγραφή |
|------------ | ------------- | ------------- | -------------|
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Συγκεντρώνει έγγραφα ομαδοποιώντας τα (εάν παρέχεται το groupBy) και εφαρμόζοντας πολλαπλές λειτουργίες. Υποστηρίζονται διαφορετικές λειτουργίες (π.χ. άθροισμα, countDistinct, μέσος όρος, κλπ.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Παλιότεροι σχολιαστές στη σελίδα που ΔΕΝ είναι επί του παρόντος συνδεδεμένοι. Ταξινομημένα κατά displayName. Χρησιμοποιήστε το μετά την εξάντληση του /users/online για να αποδώσετε μια ενότητα \"Members\". Σελιδοποίηση κατά commenterName: ο διακομιστής διασχίζει το μερικό ευρετήριο {tenantId, urlId, commenterName} από afterName προς τα εμπρός μέσω $gt, χωρίς κόστος $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Τρέχοντες online θεατές μιας σελίδας: άτομα των οποίων η συνδρομή WebSocket είναι εγγεγραμμένη στη σελίδα αυτή τη στιγμή. Επιστρέφει anonCount + totalCount (συνδρομητές σε ολόκληρο το δωμάτιο, συμπεριλαμβανομένων των ανώνυμων θεατών που δεν απαριθμούμε). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Λίστα σελίδων για έναν ενοικιαστή. Χρησιμοποιείται από τον πελάτη desktop FChat για να γεμίσει τη λίστα δωματίων του. Απαιτεί το `enableFChat` να είναι true στην επιλυμένη προσαρμοσμένη ρύθμιση για κάθε σελίδα. Σελίδες που απαιτούν SSO φιλτράρονται βάσει της προσπέλασης ομάδας του χρήστη που κάνει το αίτημα. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Συγκεντρωτικές πληροφορίες χρηστών για έναν ενοικιαστή. Δίνοντας userIds, επιστρέφει πληροφορίες εμφάνισης από User / SSOUser. Χρησιμοποιείται από το widget σχολίων για να εμπλουτίσει χρήστες που μόλις εμφανίστηκαν μέσω συμβάντος παρουσίας. Χωρίς περιβάλλον σελίδας: η ιδιωτικότητα επιβάλλεται ομοιόμορφα (ιδιωτικά προφίλ καλύπτονται). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Ανεβάστε και αλλάξτε το μέγεθος μιας εικόνας |
...
## Τεκμηρίωση για τα Μοντέλα
 ...
 - [BulkAggregateQuestionResultsRequest](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Models/Bulk