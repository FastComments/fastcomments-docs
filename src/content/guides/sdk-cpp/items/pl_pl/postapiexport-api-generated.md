...
Zwraca: [`ModerationExportResponse`](https://github.com/FastComments/fastcomments-cpp/blob/master/client/include/FastCommentsClient/model/ModerationExportResponse.h)

## Przykład

[inline-code-attrs-start title = 'postApiExport Przykład'; type = 'cpp'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
...
api->postApiExport(tenantId, options)
    .then([](std::shared_ptr<ModerationExportResponse> response) {
        if (response) {
            // obsłuż udaną odpowiedź eksportu
        }
    })
    .wait();
[inline-code-end]