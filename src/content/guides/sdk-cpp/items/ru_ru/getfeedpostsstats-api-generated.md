...
[inline-code-attrs-start title = 'Пример использования getFeedPostsStats'; type = 'cpp'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
...
api->getFeedPostsStats(tenantId, postIds, sso)
    .then([](pplx::task<std::shared_ptr<FeedPostsStatsResponse>> previous) {
        try {
            ...
            // обработайте статистику здесь (например, просмотреть поля, обновить пользовательский интерфейс)
