...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Агрегирует документы путем их группировки (если groupBy задан) и применения нескольких операций. Поддерживаются различные операции (например, sum, countDistinct, avg и др.). |
