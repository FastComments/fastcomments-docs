...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Аггрегирует документы, группируя их (если groupBy задан) и применяя несколько операций. Поддерживаются различные операции (например, sum, countDistinct, avg и др.). |
...
*DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts |  требуется tenantId afterId |
...
*PublicApi* | [**getCommentsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getCommentsPublic) | **GET** /comments/{tenantId} |  требуется tenantId urlId |
*PublicApi* | [**getEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getEventLog) | **GET** /event-log/{tenantId} |  требуется tenantId urlId userIdWS |
*PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} |  требуется tenantId afterId |
...
*PublicApi* | [**getGlobalEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getGlobalEventLog) | **GET** /event-log/global/{tenantId} |  требуется tenantId urlId userIdWS |
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Комментаторы на странице, которые сейчас НЕ находятся онлайн. Сортировка по displayName. Используется после /users/online для отображения раздела «Участники». Курсорная пагинация по commenterName: сервер проходит по частичному индексу {tenantId, urlId, commenterName} от afterName вперед с помощью $gt, нет затрат на $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Сейчас онлайн-зрители страницы: пользователи, чья сессия websocket в данный момент подписана на страницу. Возвращает anonCount + totalCount (подписчики всей комнаты, включая анонимных пользователей, которых не перечисляем). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Список страниц для арендатора. Используется десктоп-клиентом FChat для заполнения списка комнат. Требует, чтобы `enableFChat` был установлен в true в кастомной конфигурации страницы. Страницы с SSO фильтруются по группам доступа пользователя. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Массовая информация о пользователях для арендатора. По списку userIds возвращает отображаемую информацию из User / SSOUser. Используется виджетом комментариев для обогащения пользователей, появившихся по событию присутствия. Без контекста страницы: приватность применяется одинаково (закрытые профили маскируются). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Загрузить и изменить размер изображения |
