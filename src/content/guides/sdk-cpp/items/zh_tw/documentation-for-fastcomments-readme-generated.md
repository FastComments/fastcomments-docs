...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | 根據分組（如果提供 groupBy）對文件進行彙總，並應用多個操作。支援不同操作（例如 sum, countDistinct, avg 等）。 |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | 過去發表過評論但目前不在線的頁面用戶。按照 displayName 排序。在你用盡 /users/online 來渲染「成員」區塊後再使用。以 commenterName 為分頁游標：伺服器根據 {tenantId, urlId, commenterName} 索引從 afterName 依 $gt 向前移動，無 $skip 成本。 |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | 頁面當前在線的瀏覽者：就是目前 websocket 連線訂閱該頁面的人。回傳 anonCount + totalCount（全聊天室包含不具名觀看者的人數，這些匿名者不會一一列出）。 |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | 租戶的頁面列表。由桌面端 FChat 客戶端用來填充聊天室房間列表。要求已為每頁啟用 `enableFChat`。需要 SSO 的頁面會依據請求用戶群組授權進行過濾。 |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | 批次獲取租戶的使用者資訊。根據 userIds 回傳 User / SSOUser 的展示資訊。由留言元件用來豐富剛透過 presence 事件出現的用戶。沒有頁面上下文：隱私強制統一（私人檔案會遮蔽）。 |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | 啟用或停用特定留言的通知。 |
*PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | 啟用或停用頁面通知。訂閱頁面的用戶將會收到新留言根評論的通知及... |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | 上傳並調整圖片大小 |
