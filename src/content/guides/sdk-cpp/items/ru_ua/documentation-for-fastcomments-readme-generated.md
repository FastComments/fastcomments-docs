...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Агрегирует документы путем их группировки (если задан groupBy) и применения нескольких операций. Поддерживаются различные операции (например, sum, countDistinct, avg и др.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Предыдущие комментаторы на странице, которые НЕ онлайн в данный момент. Отсортированы по displayName. Используйте после получения результата /users/online для вывода раздела "Members" (Участники). Постраничная пагинация по commenterName: сервер проходит частичный индекс {tenantId, urlId, commenterName} от afterName вперед через $gt, отсутствует стоимость $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Пользователи страницы, которые находятся онлайн прямо сейчас: люди, чья websocket-сессия подписана на страницу в данный момент. Возвращает anonCount + totalCount (все участники комнаты, включая анонимных пользователей, которых мы не перечисляем). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Выводит список страниц для арендатора. Используется FChat-десктоп-клиентом для заполнения списка комнат. Требуется, чтобы на итоговой индивидуальной конфигурации для каждой страницы был включен параметр `enableFChat`. Страницы с требованием SSO фильтруются по группе доступа пользователя, делающего запрос. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Массовое получение информации о пользователях для арендатора. По заданному списку userIds возвращает отображаемую информацию из User / SSOUser. Используется виджетом комментариев для обогащения информации о пользователях, появившихся через событие присутствия. Нет контекста страницы: приватность строго соблюдается (закрытые профили маскируются). |
...
## Документация по моделям
 ...
 - [DigestEmailFrequency](https://github.com/FastComments