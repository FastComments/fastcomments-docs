...
## Документація для API-ендпоінтів
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Попередні коментатори на сторінці, які НЕ онлайн. Відсортовані за displayName. Використовуйте після вичерпання /users/online, щоб сформувати розділ «Учасники». Пагінація курсором за commenterName: сервер обходить частковий індекс {tenantId, urlId, commenterName} від afterName вперед через $gt, без витрат $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Поточні онлайн‑переглядачі сторінки: люди, чиї websocket‑сесії підписані на сторінку зараз. Повертає anonCount + totalCount (всі підписники кімнати, включаючи анонімних користувачів, які не перераховуються). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Список сторінок для орендаря. Використовується настільним клієнтом FChat для формування списку кімнат. Потребує `enableFChat` = true у специфічному налаштуванні кожної сторінки. Сторінки, що вимагають SSO, фільтруються згідно групового доступу запитуваного користувача. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Масивна інформація про користувачів орендаря. За заданими userIds повертає дані відображення з User / SSOUser. Використовується віджетом коментарів для збагачення даних користувачів, які зʼявилися через подію присутності. Без контексту сторінки: конфіденційність застосовується однорідно (приватні профілі маскуються). |
...
## Документація для моделей
 ...
 - [CreateModeratorResponse](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Models