...
| Klasse | Methode | HTTP-aanvraag | Beschrijving |
|------------ | ------------- | ------------- | -------------|
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Eerdere reageerders op de pagina die NIET momenteel online zijn. Gesorteerd op displayName. Gebruik dit na het uitputten van /users/online om een sectie "Leden" weer te geven. Cursor-paginering op commenterName: server loopt het gedeeltelijke {tenantId, urlId, commenterName} index van afterName naar voren via $gt, geen $skip-kosten. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Op dit moment online kijkers van een pagina: mensen wiens websocket-sessie nu op de pagina is geabonneerd. Geeft anonCount + totalCount terug (abonnees in de ruimte, inclusief anonieme kijkers die we niet benoemen). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Lijst met pagina's voor een tenant. Gebruikt door de FChat desktop-client om de kamerlijst in te vullen. Vereist dat `enableFChat` waar is op de gevonden custom config voor elke pagina. Pagina's waarvoor SSO vereist is worden gefilterd op de groepstoegang van de aanvragende gebruiker. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Bulk gebruikersinformatie voor een tenant. Gegeven userIds, retourneer display-informatie uit User / SSOUser. Gebruikt door de comments-widget om gebruikers te verrijken die net zijn verschenen via een aanwezigheidsevenement. Geen pagina-context: privacy wordt consequent gehandhaafd (privé-profielen worden afgeschermd). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Upload en resize een afbeelding |
...
## Documentatie voor Modellen
 ...
 - [DigestEmailFrequency](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Models/DigestEmailFrequency