...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Aggrega i documenti raggruppandoli (se groupBy è fornito) e applicando più operazioni. Sono supportate diverse operazioni (ad es. somma, countDistinct, avg, ecc.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Commentatori precedenti della pagina che NON sono attualmente online. Ordinati per displayName. Utilizzare questo endpoint dopo aver esaurito /users/online per mostrare una sezione "Membri". Paginazione del cursore su commenterName: il server cammina sull'indice parziale {tenantId, urlId, commenterName} da afterName in avanti tramite $gt, nessun costo $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Visualizzatori attualmente online di una pagina: persone la cui sessione websocket è attualmente iscritta alla pagina. Restituisce anonCount + totalCount (tutti gli iscritti, inclusi gli utenti anonimi che non vengono enumerati). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Elenca le pagine per un tenant. Utilizzato dal client desktop FChat per popolare la lista delle room. Richiede che `enableFChat` sia true nella configurazione personalizzata risolta di ogni pagina. Le pagine che richiedono SSO vengono filtrate rispetto ai permessi di gruppo dell'utente che effettua la richiesta. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Informazioni in bulk sugli utenti di un tenant. Dati gli userIds, restituisce informazioni display dall'User / SSOUser. Utilizzato dal widget dei commenti per arricchire gli utenti appena apparsi tramite evento di presenza. Nessun contesto di pagina: la privacy viene applicata uniformemente (i profili privati sono mascherati). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Carica e ridimensiona un'immagine |
...
## Documentazione per i Modelli
 ...
 - [DigestEmailFrequency](https://github.com