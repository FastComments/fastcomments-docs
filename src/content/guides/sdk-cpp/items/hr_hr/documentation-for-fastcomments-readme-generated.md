...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Grupira dokumente (ako je naveden groupBy) i primjenjuje više operacija. Podržane su različite operacije (npr. sum, countDistinct, avg itd.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Prijašnji komentatori na stranici koji TRENUTNO nisu online. Sortirano po displayName. Koristite ovo nakon što iscrpite /users/online kako biste prikazali odjeljak \"Članovi\". Paginacija kursora po commenterName: server pretražuje djelomični indeks {tenantId, urlId, commenterName} od afterName prema naprijed koristeći $gt, nema troška $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Trenutno online gledatelji stranice: osobe čija je websocket sesija trenutno prijavljena na stranicu. Vraća anonCount + totalCount (pretplatnici iz cijele sobe, uključujući anonimne koje ne nabrajamo). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Prikaz popisa stranica za zakupca. Koristi ga FChat desktop klijent za popunjavanje svog popisa soba. Zahtijeva da je `enableFChat` postavljeno na true u prilagođenoj konfiguraciji stranice. Stranice koje zahtijevaju SSO filtriraju se prema grupnom pristupu korisnika. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Skupne informacije o korisnicima za zakupca. Za dane userId-je, vraća prikaz informacija iz User / SSOUser. Koristi ga widget za komentare za obogaćivanje korisnika koji su se pojavili putem događaja prisutnosti. Nema konteksta stranice: privatnost se provodi dosljedno (privatni profili su maskirani). |
...
## Dokumentacija za modele
 ...
 - [DeletedCommentResultComment](https://github.com/FastComments/