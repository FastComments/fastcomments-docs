...
## Τεκμηρίωση για τα Τελικά Σημεία API

All URIs are relative to *https://fastcomments.com*

| Κατηγορία | Μέθοδος | HTTP αίτημα | Περιγραφή |
|------------ | ------------- | ------------- | -------------|
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Παλιοί σχολιαστές στη σελίδα που ΔΕΝ είναι αυτή τη στιγμή online. Ταξινομείται κατά displayName. Χρησιμοποιήστε το μετά την εξάντληση του /users/online για να αποδώσετε μια ενότητα \"Members\". Σελιδοποίηση με δείκτη (cursor) στο commenterName: ο διακομιστής διασχίζει το μερικό ευρετήριο {tenantId, urlId, commenterName} από afterName προς τα εμπρός μέσω $gt, χωρίς κόστος $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Προβολείς της σελίδας που είναι αυτή τη στιγμή online: άτομα των οποίων η συνεδρία websocket είναι εγγεγραμμένη στη σελίδα αυτή τη στιγμή. Επιστρέφει anonCount + totalCount (συνδρομητές σε όλο το δωμάτιο, συμπεριλαμβανομένων των ανώνυμων προβολέων που δεν απαριθμούμε). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Λίστα σελίδων για έναν ενοικιαστή. Χρησιμοποιείται από τον πελάτη FChat desktop για την εμφάνιση της λίστας δωματίων. Απαιτείται `enableFChat` να είναι true στην επιλυμένη προσαρμοσμένη διαμόρφωση για κάθε σελίδα. Σελίδες που απαιτούν SSO φιλτράρονται βάσει της πρόσβασης ομάδας του αιτούντος χρήστη. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Πλήθος πληροφοριών χρήστη για έναν ενοικιαστή. Δίνεται ένα σύνολο userIds, επιστρέφονται πληροφορίες εμφάνισης από User / SSOUser. Χρησιμοποιείται από το widget σχολίων για την ενίσχυση των χρηστών που μόλις εμφανίστηκαν μέσω γεγονότος παρουσίας. Δεν υπάρχει συμφραζόμενο σελίδας: η ιδιωτικότητα εφαρμόζεται ομοιόμορφα (ιδιωτικά προφίλ είναι καλυμμένα). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Ανέβαση και αλλαγή μεγέθους εικόνας |
...
## Τεκμηρίωση για Μοντέλα
 ...
 - [BulkCreateHashTagsBody_tags_inner](https://github.com/FastComments/fastcomments-c