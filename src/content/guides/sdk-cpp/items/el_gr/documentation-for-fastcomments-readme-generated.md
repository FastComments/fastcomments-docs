...
| Κλάση | Μέθοδος | HTTP request | Περιγραφή |
|------------ | ------------- | ------------- | -------------|
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Συγκεντρώνει έγγραφα ομαδοποιώντας τα (αν δοθεί το groupBy) και εφαρμόζοντας πολλαπλές λειτουργίες. Υποστηρίζονται διάφορες λειτουργίες (π.χ. sum, countDistinct, avg, κ.λπ.). |
...
*DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts |  απαιτείται tenantId afterId |
...
*PublicApi* | [**getCommentsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getCommentsPublic) | **GET** /comments/{tenantId} |  απαιτείται tenantId urlId |
*PublicApi* | [**getEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getEventLog) | **GET** /event-log/{tenantId} |  απαιτείται tenantId urlId userIdWS |
*PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} |  απαιτείται tenantId afterId |
...
*PublicApi* | [**getGlobalEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getGlobalEventLog) | **GET** /event-log/global/{tenantId} |  απαιτείται tenantId urlId userIdWS |
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Προηγούμενοι σχολιαστές στη σελίδα που ΔΕΝ είναι αυτή τη στιγμή online. Ταξινομημένοι κατά displayName. Χρησιμοποιήστε το αφού έχετε εξαντλήσει το /users/online για να εμφανίσετε μια ενότητα "Μέλη". Σελιδοποίηση (cursor pagination) κατά commenterName: ο server ξεκινά από το μερικό {tenantId, urlId, commenterName} index και προχωράει μπροστά μέσω $gt, χωρίς κόστος $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Τρέχοντες online θεατές μιας σελίδας: άτομα των οποίων η websocket συνεδρία είναι συνδρομημένη στη σελίδα αυτή τη στιγμή. Επιστρέφει anonCount + totalCount (συνδρομητές ανά δωμάτιο, συμπεριλαμβανομένων των ανώνυμων που δεν απαριθμούμε). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Λίστα σελίδων για έναν tenant. Χρησιμοποιείται από τον FChat desktop client για την εμφάνιση της λίστας δωματίων. Απαιτεί το `enableFChat` να είναι true στη λυμένη custom config κάθε σελίδας. Σελίδες που απαιτούν SSO φιλτράρονται με βάση την πρόσβαση του χρήστη (group access). |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Μαζική ανάκτηση πληροφοριών χρηστών για έναν tenant. Δίνοντας userIds, επιστρέφει εμφανιζόμενες πληροφορίες από User / SSOUser. Χρησιμοποιείται από το comment widget για να εμπλουτίσει χρήστες που εμφανίστηκαν μέσω γεγονότος παρουσίας. Δεν λαμβάνεται υπόψη το page context: το απόρρητο επιβάλλεται ομοιόμορφα (private profiles καλύπτονται). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Μεταφόρτωση και αλλαγή μεγέθους εικόνας |
