...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Commentateurs précédents sur la page qui NE sont PAS actuellement en ligne. Triés par displayName. Utilisez ceci après avoir épuisé /users/online pour afficher une section « Membres ». Pagination par curseur sur commenterName : le serveur parcourt l’index partiel {tenantId, urlId, commenterName} à partir de afterName via $gt, sans coût de $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Visualisateurs actuellement en ligne d’une page : personnes dont la session websocket est abonnée à la page en ce moment. Retourne anonCount + totalCount (abonnés dans la salle, y compris les spectateurs anonymes qui ne sont pas énumérés). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Liste des pages pour un locataire. Utilisé par le client FChat desktop pour remplir sa liste de salons. Nécessite que `enableFChat` soit vrai dans la configuration personnalisée résolue pour chaque page. Les pages nécessitant SSO sont filtrées en fonction des groupes auxquels l’utilisateur demandeur a accès. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Informations d’utilisateur en lot pour un locataire. Si on fournit userIds, retourne les infos d’affichage depuis User / SSOUser. Utilisé par le widget de commentaires pour enrichir l’affichage des utilisateurs reçus par événement de présence. Aucun contexte de page : la confidentialité est appliquée de façon uniforme (profils privés masqués). |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | Activer ou désactiver les notifications pour un commentaire précis. |
