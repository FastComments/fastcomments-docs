...
| *DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | groupBy가 제공된 경우 문서를 그룹화하고 여러 작업을 적용하여 문서를 집계합니다. 다양한 작업(예: sum, countDistinct, avg 등)이 지원됩니다. |
...
| *DefaultApi* | [**getFeedPosts**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/DefaultApi.md#getFeedPosts) | **GET** /api/v1/feed-posts | tenantId와 afterId가 필요합니다. |
...
| *PublicApi* | [**getCommentsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getCommentsPublic) | **GET** /comments/{tenantId} | tenantId와 urlId가 필요합니다. |
| *PublicApi* | [**getEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getEventLog) | **GET** /event-log/{tenantId} | tenantId, urlId 및 userIdWS가 필요합니다. |
| *PublicApi* | [**getFeedPostsPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getFeedPostsPublic) | **GET** /feed-posts/{tenantId} | tenantId와 afterId가 필요합니다. |
...
| *PublicApi* | [**getGlobalEventLog**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getGlobalEventLog) | **GET** /event-log/global/{tenantId} | tenantId, urlId 및 userIdWS가 필요합니다. |
| *PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | 현재 온라인이 아닌 페이지의 이전 댓글 작성자들. displayName 순으로 정렬됩니다. /users/online을 모두 사용한 뒤 \"멤버\" 섹션을 렌더링할 때 사용합니다. commenterName에 대한 커서 페이지네이션: 서버가 afterName 이후로 $gt를 사용해 부분 {tenantId, urlId, commenterName} 인덱스를 탐색하며, $skip 비용이 없습니다. |
| *PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | 페이지의 현재 온라인 시청자: 현재 해당 페이지에 웹소켓 세션이 구독된 사람들. anonCount와 totalCount를 반환합니다 (방 전체 구독자 수, 여기에는 열거되지 않는 익명 시청자도 포함). |
| *PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | 테넌트의 페이지 목록을 반환합니다. FChat 데스크톱 클라이언트가 방 목록을 채우는 데 사용됩니다. 각 페이지에 대해 해결된 커스텀 설정에서 `enableFChat`이 true여야 합니다. SSO가 필요한 페이지는 요청 사용자의 그룹 접근 권한에 따라 필터링됩니다. |
...
| *PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | 테넌트에 대한 대량 사용자 정보. userIds를 제공하면 User / SSOUser에서 표시 정보를 반환합니다. 댓글 위젯이 존재 이벤트를 통해 새로 나타난 사용자를 풍부하게 만들 때 사용됩니다. 페이지 컨텍스트가 없으므로 프라이버시가 일관되게 적용됩니다 (비공개 프로필은 마스킹됩니다). |
...
| *PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | 특정 댓글에 대한 알림을 활성화하거나 비활성화합니다. |
| *PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | 사용자가 페이지에 가입하면 새 루트 댓글에 대한 알림이 생성됩니다. 또한 |
...
| *PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | 이미지 업로드 및 리사이즈 |
...
## 모델 문서
 ...
 - [APITicketDetail](https://github.com/FastComments/fastcomments-cpp/blob/master/docs/Models/APITicketDetail