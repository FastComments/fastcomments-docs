...
此函式庫包含三個模組。產生的 API 用戶端、包含手寫實用工具以便更輕鬆使用 API 的核心 Java 函式庫，以及 `pubsub` 模組（用於訂閱變更串流的函式庫）。
...
在 API 用戶端中，有兩個類別：`DefaultApi` 與 `PublicApi`。`DefaultApi` 包含需要您 API 金鑰的方法，而 `PublicApi` 則包含可直接從瀏覽器、行動裝置等在不需驗證的情況下呼叫的 API。