FastComments поддерживает ежедневный, еженедельный или ежемесячный электронный дайджест по электронной почте для модераторов и администраторов.
...
Помимо общей статистики по комментариям, он также будет перечислять три самых последних комментария, требующих проверки.

Для каждого из этих комментариев предоставляются прямые «волшебные» ссылки для:
- Одобрить комментарий.
- Отметить комментарий как просмотренный и перейти на страницу ответа.
- Отметить комментарий как спам.

Эти ссылки для каждого комментария автоматически аутентифицируют вас и выполнят действие прямо из вашего письма.

Кроме того, в дайджесте находится кнопка «Moderate Comments», которая выполнит ту же аутентификацию и перенаправит вас на страницу «Moderate Comments».

Обратите внимание, что эти «волшебные» ссылки истекают через некоторое время.
...
FastComments отправляет несколько типов писем модераторам и администраторам. При желании можно отказаться от уведомлений `Comment Reply`, при этом
продолжая получать уведомления `New Comment` — выбрав соответствующие параметры на странице `Edit Notifications`, показанной выше.
