FastComments prend en charge un récapitulatif par e-mail quotidien, hebdomadaire ou mensuel pour les modérateurs et les administrateurs.
...
En plus d'inclure des statistiques globales sur vos commentaires, il listera également les trois commentaires les plus récents nécessitant un examen.

Pour chacun de ces commentaires, des liens magiques directs sont fournis pour :
- Approuver le commentaire.
- Marquer le commentaire comme examiné et aller sur la page de réponse.
- Marquer le commentaire comme spam.

Ces liens pour chaque commentaire vous authentifieront automatiquement et effectueront l'action directement depuis votre e-mail.

De plus, un bouton Modérer les commentaires se trouve dans le récapitulatif et effectuera la même authentification pour vous amener sur la page Modérer les commentaires.
...
FastComments envoie plusieurs types d'e-mails aux modérateurs et aux administrateurs. Si vous le souhaitez, il est possible de se désinscrire des notifications `Comment Reply`, tout en continuant à recevoir les notifications `New Comment` en choisissant les options appropriées dans la page `Edit Notifications` montrée ci-dessus.
