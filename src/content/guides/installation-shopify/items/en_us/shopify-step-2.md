The first step to using FastComments with Shopify is disabling Shopify's comment system.

Note: FastComments Support can assist you in migrating existing comment threads.

To replace the Shopify comment system, go to your store dashboard and click `Blog posts` in the bottom left:
...
Then click `Manage blogs` in the top right:
...
Click the blog you want to manage. If it says `Comments are disabled`, you can skip this step and move to `Step Two`.
...
If not, select the blog and make sure `Comments are disabled` is selected in the right panel:
