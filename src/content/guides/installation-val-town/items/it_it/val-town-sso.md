...
export function buildSSOPayload(user) {
  // l'id deve essere stabile per la stessa persona, altrimenti ottengono una nuova identità di commento ad ogni login.
  const id = `vt-${user.id}`;

  return new SecureSSOPayloadBuilder(Deno.env.get("FASTCOMMENTS_API_SECRET"), {
    id,
    // l'email è obbligatoria e deve essere unica.
    email: user.email ?? `${id}@users.noreply.val.town`,
    // il nome utente è obbligatorio e non può essere un'email.
...
[inline-code-attrs-start title = 'Configurazione del widget con SSO'; type='javascript' inline-code-attrs-end]
[inline-code-start]
...
app.get("/", async (c) => {
  ...
  // ...renderizza il widget con questa configurazione
});
