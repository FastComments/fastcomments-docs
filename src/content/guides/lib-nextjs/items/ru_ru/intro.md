Это официальная библиотека Next.js для FastComments.

Встраивайте виджеты комментариев в реальном времени, чата и отзывов в ваше приложение Next.js.

## Репозиторий

[Посмотреть на GitHub](https://github.com/FastComments/fastcomments-nextjs)

[![npm](https://img.shields.io/npm/v/fastcomments-nextjs?logo=npm&label=npm&color=cb3837)](https://www.npmjs.com/package/fastcomments-nextjs)

---