...
- User names, emails, and avatars are passed securely to FastComments.
...
User data (name, email, avatar) is sent client-side without a cryptographic signature. This is easier to set up since it does not require an API Secret, but it is less secure because user data is not verified server-side.
