...
将 ZIP 解压到您的 Moodle 安装目录，使插件位于 `<moodle-root>/local/fastcomments`。插件目录中应直接包含 `version.php`、`lib.php` 和其他插件文件（不要嵌套在子文件夹中）。
...
以站点管理员身份登录并导航到 **站点管理 > 通知**。Moodle 会检测到新插件并提示您运行安装。

#### 配置插件

安装后，转到 **站点管理 > 插件 > 本地插件 > FastComments** 输入您的设置。有关每个选项的详细信息，请参阅 [配置](#moodle-configuration) 部分。