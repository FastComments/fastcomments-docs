...
Sous « Associer au domaine » - sélectionnez votre domaine « localhost ».

**REMARQUE : Alternativement, vous pouvez utiliser un secret API pour toutes les activités de test et les environnements de staging. Il suffit d'ajouter un secret API pour "All Domains", et de lui donner un nom comme "test".**

Assurez-vous d'avoir un secret API défini pour votre(s) domaine(s) de production. Les événements pour tous les autres domaines utiliseront le secret générique (de test).

### Étape 3 : Ajoutez votre webhook

Lorsque vous exécutez ngrok ou un outil similaire, définissez la valeur pour "localhost" [ici](https://fastcomments.com/auth/my-account/manage-data/webhooks).
...
Vous pouvez maintenant ajouter, modifier ou supprimer des commentaires et devriez nous voir appeler votre machine de développement locale avec les événements, en utilisant votre clé API de test. Il peut y avoir un délai allant jusqu'à 30 secondes
pour que les événements arrivent sur votre machine.