...
|-------|----------|-------------|
| `url` | Да | Абсолютный http или https URL. |
| `event` | Да | `comment-created`, `comment-updated` or `comment-deleted`. |
| `domain` | Нет | A domain from your account configuration. Defaults to `*`, which receives events for every domain. |
| `method` | Нет | `POST` (default), `PUT` or `DELETE`. |
