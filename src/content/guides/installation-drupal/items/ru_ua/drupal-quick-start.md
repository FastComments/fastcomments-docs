...
2. Включите его с помощью `drush en fastcomments`, или через интерфейс администратора по адресу `/admin/modules`.
3. Перейдите в `Administration > Configuration > Content > FastComments` (`/admin/config/content/fastcomments`).
4. Введите ваш Tenant ID и API Secret из [Настройки > API/SSO](https://fastcomments.com/auth/my-account/api) ([EU](https://eu.fastcomments.com/auth/my-account/api)).
