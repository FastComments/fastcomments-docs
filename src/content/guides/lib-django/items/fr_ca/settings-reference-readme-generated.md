...
| `API_KEY` | `""` | Votre secret d'API. Signe le SSO sécurisé et authentifie `admin()`. |
...
| `SSO.LOGIN_URL` / `SSO.LOGOUT_URL` | `None` | Affiché aux visiteurs déconnectés ; valeur par défaut `reverse("login"/"logout")`. |
