コードスニペットはコードの小さな断片、いわゆるスニペットで、コメント中に共有できます。
...
FastCommentsでは、コードのスニペットを貼り付けるだけで自動的に検出され、フォーマットされます。

コメントする際、コードスニペットは `<code>` ブロックでラップされ、フォーマットはコメントを送信した後に行われます。

多くの言語では、コードスニペットを貼り付けると `<code>` ブロックで自動的に囲まれます。

あるいは、テキストの一部をハイライトしてツールバーの `Code` ボタンをクリックすることもできます。
