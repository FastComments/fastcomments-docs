Ensuite, configurons les choses afin que le fil de commentaires change en fonction de la page courante, permettant aux utilisateurs de discuter du contenu affiché.

Sans les étapes suivantes, vous n'aurez qu'un seul fil de commentaires global pour l'ensemble de votre site - ce qui n'est pas très utile.

#### Mode développeur

Pour ajouter cette fonctionnalité, nous devons entrer dans ce que Wix appelle `Dev Mode`.
...
Nous allons ajouter du code personnalisé pour y parvenir, mais d'abord nous devons donner un ID à la nouvelle élément intégré afin d'y faire référence.

Appelons-le `fastcomments`.

Cliquez sur le nouvel élément intégré que nous avons ajouté, et en mode développeur dans le coin inférieur droit vous devriez voir un champ ID avec une valeur comme `html1`:
...
Nous pouvons maintenant ajouter notre code personnalisé qui indique à la zone de commentaires quelle page nous consultons.

En bas de l'écran, vous devriez voir un éditeur de code comme ceci:

<div class="screenshot white-bg">
    <div class="title">Ouvrir l'éditeur</div>
    <img class="screenshot-image" src="/images/installation-guides/installation-guide-wix-step-7-open-editor.png" alt="Définir l'ID" />
</div>

Copiez le code suivant et collez-le dedans:

[inline-code-attrs-start title = 'Extrait de navigation des commentaires Wix'; type = 'javascript'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
