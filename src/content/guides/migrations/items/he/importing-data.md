...
By navigating [כאן](https://fastcomments.com/auth/my-account/manage-data/import) we can upload the file containing the data to migrate.

[app-screenshot-start url='/auth/my-account/manage-data/import'; selector = '.account-block'; alt='דף ייבוא FastComments עם בחירת הספק ושדות העלאת קובץ עבור קובץ ייצוא'; title='טופס דף הייבוא' app-screenshot-end]
...
[app-screenshot-start url='/auth/my-account/manage-data/import?demo=true'; selector = '.content'; alt='דף ייבוא המציג משימת ייבוא רצה ואת המצב המדווח על ידי מערכת עיבוד המשימות'; title='מצב משימת הייבוא' app-screenshot-end]
