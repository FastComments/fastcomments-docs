...
<script>
    (function () {
        ...
        // Συνάρτηση για να διασφαλίσει ότι το script έχει φορτωθεί
        function ensureScriptLoaded() {
            return new Promise((resolve) => {
                // Έλεγχος εάν το script tag υπάρχει ήδη
                ...
                if (!scriptTag) {
                    ...
                    scriptTag.onerror = () => {
                        ...
                        resolve(); // Επιλύεται ούτως ή άλλως για να αποφευχθεί το κρέμασμα
                    };
    
                    document.head.appendChild(scriptTag);
                } else if (window.FastCommentsUI) {
                    // Το tag του script υπάρχει και έχει ήδη φορτωθεί
                    console.log('FastComments: Script already loaded');
                    resolve();
                } else {
                    // Το tag του script υπάρχει αλλά δεν είναι ακόμη έτοιμο
                    console.log('FastComments: Waiting for script to initialize...');
                    scriptTag.addEventListener('load', () => {
                        resolve();
                    });
    
                    // Εναλλακτική σε περίπτωση που το script φορτώνει ήδη
        ...
        // Κύρια συνάρτηση απόδοσης (render)
        async function render() {
            ...
            function tryNext() {
                ...
                if (container) {
                    // Διπλός έλεγχος εάν το FastCommentsUI είναι διαθέσιμο
                    if (!window.FastCommentsUI) {
                    ...
                    // Λάβετε το τρέχον URL ως urlId
                    const newUrlId = window.location.pathname;
    
                    // Ελέγξτε αν χρειάζεται να ξανααποδώσετε (αλλαγή urlId ή πρώτη απόδοση)
                    if (currentUrlId !== newUrlId || !lastInstance) {
                        currentUrlId = newUrlId;
    
                        // Καταστρέψτε την προηγούμενη παρουσία αν υπάρχει
                        if (lastInstance) {
                    ...
                    // Παρακολούθηση εάν αφαιρεθεί το container ή αλλάξει το URL
        ...
        // Επαναφορά απόδοσης όταν αλλάζει η τοποθεσία (για SPA)
