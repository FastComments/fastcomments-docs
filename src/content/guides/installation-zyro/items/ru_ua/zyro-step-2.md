...
и перезагрузите эту страницу, если нет — тогда код будет предварительно заполнен данными вашей учётной записи, иначе будет показан демонстрационный код.
...
Важно использовать приведённый выше код, а не фрагменты кода из другой документации, так как этот сниппет специально подготовлен для Zyro.

Теперь у вас должно быть что-то похожее на следующее, что выглядит пустым. Это ожидаемо. Наведите курсор мыши на область,
где должен находиться виджет:
...
Теперь перетащите виджет, задав нужный размер — вы увидите, как он появится:
