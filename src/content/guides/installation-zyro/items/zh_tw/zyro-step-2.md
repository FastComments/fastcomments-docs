...
複製下方程式碼。請確保您已登入 [fastcomments.com](https://fastcomments.com)
並在未登入時重新載入此頁面，這樣程式碼才會預先填入您的帳戶資訊，否則會顯示範例程式碼。
...
重要的是您要使用上面的程式碼，而不是其他文件中的程式碼片段，因為此片段是專為 Zyro 製作的。

此時您應該會看到類似下方的畫面，看起來是空白的。這是預期的。將滑鼠移到小工具應該出現的區域：
