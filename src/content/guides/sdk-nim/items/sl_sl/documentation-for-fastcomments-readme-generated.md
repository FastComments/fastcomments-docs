...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Združi dokumente z njihovim grupiranjem (če je podan parameter groupBy) in uporabo več operacij. Podprte so različne operacije (npr. sum, countDistinct, avg itd.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Prejšnji komentatorji na strani, KI NI Trenutno online. Razvrščeni po displayName. Uporabite to po izčrpanju /users/online za prikaz razdelka „Člani“. Premikanje po kazalniku (cursor) na commenterName: strežnik sprehaja delni indeks {tenantId, urlId, commenterName} od afterName naprej z $gt, brez $skip stroškov. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Trenutno‑online gledalci strani: ljudje, katerih WebSocket seja je trenutno naročena na to stran. Vrne anonCount + totalCount (število naročnikov v sobi, vključno z anonimiziranimi gledalci, ki jih ne naštejemo). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Seznam strani za najemnika. Uporablja ga namizni odjemalec FChat za napolnitev seznama sob. Zahteva, da je `enableFChat` nastavljen na true v razrešenih prilagojenih nastavitvah za vsako stran. Strani, ki zahtevajo SSO, se filtrirajo glede na skupinski dostop zahtevajočega uporabnika. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Zbirni uporabniški podatki za najemnika. Za podane userIds se vrnejo prikazni podatki iz User / SSOUser. Uporablja jih vtičnik komentarjev za obogatitev uporabnikov, ki se pravkar pojavijo preko dogodka prisotnosti. Brez konteksta strani: zasebnost je enotno uveljavljena (zasebni profili so maskirani). |
...
## Dokumentacija za modele
 ...
 - [CreateHashTagResponse](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Models/Create