...
Όλα τα URI είναι σχετικά με το *https://fastcomments.com*
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Συγκεντρώνει έγγραφα ομαδοποιώντας τα (αν έχει δοθεί το groupBy) και εφαρμόζοντας πολλαπλές λειτουργίες. Υποστηρίζονται διάφορες λειτουργίες (π.χ. άθροιση, countDistinct, avg, κ.ά.). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Προηγούμενοι σχολιαστές στη σελίδα που ΔΕΝ είναι αυτή τη στιγμή συνδεδεμένοι. Ταξινομημένοι ανά displayName. Χρησιμοποιήστε το αφού εξαντλήσετε το /users/online για να εμφανίσετε μια ενότητα "Μέλη". Σελιδοποίηση cursors στο commenterName: ο server διατρέχει το μερικό ευρετήριο {tenantId, urlId, commenterName} από το afterName και μετά μέσω $gt, χωρίς κόστος $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Προβολή για όσους είναι online αυτή τη στιγμή σε μια σελίδα: άτομα των οποίων η websocket συνεδρία είναι εγγεγραμμένη αυτή τη στιγμή στη σελίδα. Επιστρέφει anonCount + totalCount (συνδρομητές του δωματίου, περιλαμβανομένων των ανώνυμων που δεν απαριθμούμε). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Λίστα σελίδων για έναν tenant. Χρησιμοποιείται από τον FChat desktop client για να γεμίσει τη λίστα δωματίων του. Απαιτεί το `enableFChat` να είναι true στα custom config που επιλύονται για κάθε σελίδα. Σελίδες που απαιτούν SSO φιλτράρονται με βάση τα δικαιώματα του χρήστη που κάνει το αίτημα. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Μαζική επιστροφή στοιχείων χρηστών για έναν tenant. Δίνοντάς userIds, επιστρέφει στοιχεία εμφάνισης από User / SSOUser. Χρησιμοποιείται από το widget σχολίων για να εμπλουτίσει χρήστες που μόλις εμφανίστηκαν μέσω event παρουσίας. Χωρίς context σελίδας: η ιδιωτικότητα εφαρμόζεται ομοιόμορφα (τα ιδιωτικά προφίλ καλύπτονται). |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | Ενεργοποίηση ή απενεργοποίηση ειδοποιήσεων για συγκεκριμένο σχόλιο. |
*PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | Ενεργοποίηση ή απενεργοποίηση ειδοποιήσεων για μια σελίδα. Όταν οι χρήστες είναι συνδρομητές σε μια σελίδα, δημιουργούνται ειδοποιήσεις για νέα root σχόλια, και επίσης |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Ανεβάστε και αλλάξτε μέγεθος σε μια εικόνα |
...
## Τεκμηρίωση για τα Μοντέλα
 ...
 - [DeleteFeedPostPublicResponse](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Models/DeleteFeed