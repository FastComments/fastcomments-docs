...
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Personnes actuellement en ligne sur une page : personnes dont la session websocket est actuellement abonnée à la page. Retourne anonCount + totalCount (abonnés à la salle, incluant les utilisateurs anonymes qui ne sont pas énumérés). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Liste les pages pour un locataire. Utilisé par le client de bureau FChat pour remplir sa liste de salles. Nécessite que `enableFChat` soit défini à true dans la configuration personnalisée résolue pour chaque page. Les pages nécessitant SSO sont filtrées selon les groupes de l'utilisateur demandeur. |
