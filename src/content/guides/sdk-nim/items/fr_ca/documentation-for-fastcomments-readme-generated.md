...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Commentateurs précédents de la page qui ne sont PAS actuellement en ligne. Triés par displayName. Utilisez ceci après avoir épuisé /users/online pour rendre une section « Membres ». Pagination par curseur sur commenterName : le serveur parcourt l’index partiel {tenantId, urlId, commenterName} à partir de afterName en avant via $gt, sans coût $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Spectateurs actuellement en ligne d’une page : personnes dont la session websocket est abonnée à la page en ce moment. Retourne anonCount + totalCount (abonnés à toute la salle, y compris les spectateurs anonymes que nous n’énumérons pas). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Répertorie les pages d’un locataire. Utilisé par le client de bureau FChat pour remplir sa liste de salles. Nécessite que `enableFChat` soit vrai dans la configuration personnalisée résolue pour chaque page. Les pages qui nécessitent SSO sont filtrées selon l’accès aux groupes de l’utilisateur demandeur. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Informations d’utilisateurs en masse pour un locataire. Donnés des userIds, renvoie les informations d’affichage provenant de User / SSOUser. Utilisé par le widget de commentaire pour enrichir les utilisateurs qui viennent d’apparaître via un événement de présence. Aucun contexte de page : la confidentialité est appliquée uniformément (les profils privés sont masqués). |
...
## Documentation des modèles
 ...
 - [BulkPreBanParams](https://github.com/FastComments/fastcomments