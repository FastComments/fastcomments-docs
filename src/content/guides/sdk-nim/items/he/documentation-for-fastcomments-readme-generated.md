...
כל הכתובות הן יחסיות ל-*https://fastcomments.com*
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | מרכז מסמכים על ידי קיבוצם (אם סופק groupBy) ויישום של מספר פעולות. פעולות שונות (לדוג' sum, countDistinct, avg, וכדומה) נתמכות. |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | מגיבים קודמים בדף שאינם כרגע מחוברים. ממוין לפי displayName. השתמש בזה אחרי שמוצו /users/online כדי להציג את אזור ה\"חברים\". פאגינציה לפי שם מגיב: השרת עובר על האינדקס {tenantId, urlId, commenterName} מהשם שאחריו, ​​באמצעות $gt, ללא עלות $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | צופים מחוברים כרגע לעמוד: אנשים שמנויים עם websocket לעמוד זה ברגע זה. מחזיר anonCount + totalCount (כל המנויים, כולל צופים אנונימיים שלא מונים אותם בשמות). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | הצגת רשימת דפים לדייר. השימוש נעשה על ידי לקוח שולחן העבודה של FChat להצגת רשימת החדרים. מחייב ש-enableFChat יהיה true בהגדרה המותאמת לדף. דפים שדורשים SSO מסוננים לפי הקבוצות אליהן למבקש יש גישה. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | פרטי משתמש מרוכזים לדייר. בהתבסס על userIds, מוחזר מידע תצוגה מ-User / SSOUser. משמש את הווידג\'ט תגובות להעשרת משתמשים שהופיעו דרך אירוע נוכחות. אין הקשר עמוד: פרטיות נאכפת באופן שווה (פרופילים פרטיים מוסתרים). |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | העלה ושנה גודל של תמונה |
