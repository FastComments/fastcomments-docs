...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | אוגר מסמכים על ידי קיבוץ שלהם (אם מסופק groupBy) והחלת פעולות מרובות. תומכות במגוון פעולות (למשל sum, countDistinct, avg וכו'). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | מגיבים קודמים בעמוד שאינם מחוברים כרגע. ממוינים לפי displayName. השתמש בזה לאחר שמיצית את /users/online כדי להציג מדור \"חברים\". דף עימוד על שם המגיב: השרת הולך באינדקס החלקי {tenantId, urlId, commenterName} מ‑afterName קדימה באמצעות $gt, ללא עלות $skip. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | צופים מחוברים כעת בעמוד: אנשים שה‑websocket שלהם מנוי לעמוד ברגע זה. מחזירים anonCount + totalCount (מנויים בכל החדר, כולל צופים אנונימיים שאינם ממוינים). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | רשימת דפים לשוכר. משמש על ידי לקוח שולחן העבודה של FChat למלא את רשימת החדרים שלו. דורש `enableFChat` להיות true בתצורה המותאמת המופקת עבור כל דף. דפים שמחייבים SSO מסוננים בהתאם לגישת הקבוצה של המשתמש המבקש. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | מידע נרחב על משתמשים לשוכר. בהתבסס על userIds, מחזיר פרטי תצוגה מ‑User / SSOUser. משמש על ידי וידג'ט ההודעה כדי להעשיר משתמשים שהופיעו לאחרונה דרך אירוע נוכחות. ללא הקשר דף: הפרטיות מוחלת באופן אחיד (פרופילים פרטיים מוסתרים). |
