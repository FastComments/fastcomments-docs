...
[inline-code-start]
let (response, httpResponse) = client.aggregate(tenantId = "my-tenant-123", aggregationRequest = AggregationRequest(), parentTenantId = "", includeStats = true)
if response.isSome:
  let aggregation = response.get()
  echo "Received aggregation for tenant my-tenant-123"
else:
  echo "No aggregation returned; HTTP response = ", httpResponse
[inline-code-end]
