...
## Документація для API-ендоинтів

All URIs are relative to *https://fastcomments.com* → **Усі URI є відносними до *https://fastcomments.com*** 
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Скуповує документи шляхом групування їх (якщо вказано **groupBy**) та застосування декількох операцій. Підтримуються різні операції (наприклад, **sum**, **countDistinct**, **avg** тощо). |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Користувачі, які раніше коментували сторінку, але зараз **НЕ** в онлайн. Сортовано за **displayName**. Використовуйте це після того, як вичерпані дані **/users/online**, щоб відобразити розділ “Учасники”. Пагінація за курсором по **commenterName**: сервер проходить частковий індекс **{tenantId, urlId, commenterName}** від **afterName** вперед через **$gt**, без витрат на **$skip**. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Користувачі, які зараз онлайн і переглядають сторінку: люди, чия **WebSocket**‑сесія підписана на сторінку в даний момент. Повертає **anonCount** + **totalCount** (всі підписники кімнати, включаючи анонімних глядачів, які не перераховуються). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Список сторінок для орендаря. Використовується десктоп‑клієнтом **FChat** для заповнення списку кімнат. Потрібно, щоб **`enableFChat`** був встановлений у **true** у розв’язаній кастомній конфігурації для кожної сторінки. Сторінки, що потребують **SSO**, фільтруються відповідно до групового доступу запитуючого користувача. |
...
## Документація моделей
 ...
 - [BulkCreateHashTagsBody](https://github.com/FastComments/fast