...
| *DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Agregira dokumente grupiranjem (ako je naveden groupBy) i primjenom višestrukih operacija. Podržane su različite operacije (npr. sum, countDistinct, avg, itd.). |
...
| *PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline |  Prethodni komentatori na stranici koji NISU trenutno online. Sortirano po displayName. Koristite ovo nakon iscrpljivanja /users/online za prikaz odjeljka „Članovi“. Paginacija po ime komentatora: poslužitelj prolazi kroz parcijalni indeks {tenantId, urlId, commenterName} od afterName naprijed putem $gt, bez $skip troška. |
| *PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online |  Trenutno online gledatelji stranice: ljudi čija websocket sesija trenutno pretplaćena na stranicu. Vraća anonCount + totalCount (pretplatnici cijele sobe, uključujući anonimne gledatelje koje ne nabrajamo). |
| *PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} |  Popis stranica za najmodavca. Koristi se u FChat desktop klijentu za popunjavanje popisa soba. Potrebno je da `enableFChat` bude true u razriješenoj prilagođenoj konfiguraciji za svaku stranicu. Stranice koje zahtijevaju SSO filtrirane su prema grupnom pristupu korisnika koji zahtijeva. |
...
| *PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info |  Skupni podaci o korisnicima za najmodavca. Dajući userIds, vraća prikazne informacije iz User / SSOUser. Koristi se u widgetu komentara za obogaćivanje korisnika koji su upravo pojavljeni putem događaja prisutnosti. Nema kontekst stranice: privatnost se jednako primjenjuje (privatni profili maskirani su). |
...
## Dokumentacija za modele
 ...
 - [CreateHashTagBody](https://github.com/FastComments/fastcomments-n