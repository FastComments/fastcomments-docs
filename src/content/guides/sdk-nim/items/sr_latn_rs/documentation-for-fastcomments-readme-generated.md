...
## Dokumentacija za API Endpointe
...
*DefaultApi* | [**aggregate**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/DefaultApi.md#aggregate) | **POST** /api/v1/aggregate | Agregira dokumente grupisanjem (ako je [groupBy] naveden) i primenom više operacija. Različite operacije (npr. sum, countDistinct, avg, itd.) su podržane. |
...
*PublicApi* | [**getOfflineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOfflineUsers) | **GET** /pages/{tenantId}/users/offline | Bivši komentatori na stranici koji TRENUTNO NISU onlajn. Sortirano po displayName. Koristite ovo nakon što iscrpite /users/online da prikažete sekciju „Članovi”. Kursor paginacija po commenterName: server prolazi delimični {tenantId, urlId, commenterName} indeks od afterName unapred preko $gt, nema $skip troškova. |
*PublicApi* | [**getOnlineUsers**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getOnlineUsers) | **GET** /pages/{tenantId}/users/online | Trenutno-onlajn posetioci stranice: ljudi čija websocket sesija je trenutno pretplaćena na stranicu. Vraća anonCount + totalCount (pretplatnici za celu sobu, uključujući anonimne posetioce koje ne navodimo pojedinačno). |
*PublicApi* | [**getPagesPublic**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getPagesPublic) | **GET** /pages/{tenantId} | Nabroji stranice za tenant-a. Koristi ga FChat desktop klijent za popunjavanje liste soba. Potrebno je da je `enableFChat` podešen na true u pronađenoj prilagođenoj konfiguraciji za svaku stranicu. Stranice koje zahtevaju SSO se filtriraju na osnovu pristupa korisničkim grupama korisnika koji šalje zahtev. |
...
*PublicApi* | [**getUsersInfo**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#getUsersInfo) | **GET** /pages/{tenantId}/users/info | Masovno dohvatanje korisničkih informacija za tenant-a. Za zadate userIds, vrati prikaz iz User / SSOUser. Koristi ga vidžet za komentare kao dopunu kod korisnika koji se pojavljuju putem događaja prisustva. Nema kontekst stranice: privatnost se primenjuje uniformno (privatni profili su maskirani). |
...
*PublicApi* | [**updateUserNotificationCommentSubscriptionStatus**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#updateUserNotificationCommentSubscriptionStatus) | **POST** /user-notifications/{notificationId}/mark-opted/{optedInOrOut} | Omogući ili onemogući obaveštenja za određeni komentar. |
*PublicApi* | [**updateUserNotificationPageSubscriptionStatus**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#updateUserNotificationPageSubscriptionStatus) | **POST** /user-notifications/set-subscription-state/{subscribedOrUnsubscribed} | Omogući ili onemogući obaveštenja za stranicu. Kada su korisnici pretplaćeni na stranicu, obaveštenja se prave za nove osnovne komentare, kao i |
...
*PublicApi* | [**uploadImage**](https://github.com/FastComments/fastcomments-nim/blob/master/docs/Apis/PublicApi.md#uploadImage) | **POST** /upload-image/{tenantId} | Otpremi i promeni veličinu slike |
...
## Dokumentacija za Modele
 ...
 - [DeleteSubscriptionAPIResponse](https://github.com/Fast