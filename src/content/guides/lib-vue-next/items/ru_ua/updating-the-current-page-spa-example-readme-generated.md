В FastComments идентификатор статьи или страницы, к которой привязаны комментарии, мы называем URL ID, так как это может быть URL или ID.
Определите URL ID следующим образом. Компонент отслеживает изменения в объекте config и будет перезагружаться, поэтому вы можете обновлять URL ID.
