...
Collez l'URL d'enregistrement FastComments dans ce champ. Laissez les autres champs vides — ils sont remplis automatiquement par FastComments pendant la procédure d'enregistrement.
...
Le nouvel outil apparaît dans la liste de vos outils LTI Advantage. Par défaut, Brightspace marque les nouveaux outils comme **désactivé** — basculez le commutateur sur **activé** afin que vos cours puissent l'utiliser.

#### Ajouter un déploiement

Dans Brightspace, les outils LTI requièrent un **déploiement** avant de pouvoir être utilisés dans les cours :

1. Ouvrez l'outil FastComments nouvellement enregistré.
2. Cliquez sur **View Deployments** > **New Deployment**.
3. Donnez un nom au déploiement (par ex. "FastComments - All Courses"), choisissez les unités d'organisation où il doit être disponible, puis enregistrez.

Après le premier lancement via ce déploiement, FastComments attache le `deployment_id` à son enregistrement de configuration - les lancements ultérieurs depuis un autre déploiement sous le même client seront rejetés à moins que vous ne vous réenregistriez.