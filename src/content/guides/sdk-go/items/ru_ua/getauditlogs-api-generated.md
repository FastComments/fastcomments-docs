...
Возвращает: [`GetAuditLogs200Response`](https://github.com/FastComments/fastcomments-go/blob/master/client/model_get_audit_logs_200_response.go)

## Пример

[inline-code-attrs-start title = 'Пример GetAuditLogs'; type = 'go'; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
package main
...
func main() {
	...
	order := openapiclient.SORT_DIR("ASC") // SORTDIR |  (необязательно)
	after := float64(1.2) // float64 |  (необязательно)
	before := float64(1.2) // float64 |  (необязательно)
	...
	// ответ от `GetAuditLogs`: GetAuditLogs200Response
