...
- **Default daily and monthly budget caps.** You can lower these per-agent; raising the per-account cap requires a plan with a higher ceiling. See [Budgets Overview](#budgets-overview).
...
The agent admin pages are gated behind the **Customization Admin** dashboard role. **Comment Moderator Admins** can review and decide approvals (see [Approval Workflow](#approval-workflow)) but cannot create or edit agents. **Billing Admins** receive [budget alert emails](#budget-alerts) regardless of whether they have agent access.
