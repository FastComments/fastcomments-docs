Агрегує документи, групуючи їх (якщо передано `groupBy`) і застосовуючи кілька операцій. Підтримуються різні операції (наприклад, `sum`, `countDistinct`, `avg` тощо).
...
[inline-code-attrs-start title = 'aggregate Приклад'; type = ''; isFunctional = false; inline-code-attrs-end]
[inline-code-start]
