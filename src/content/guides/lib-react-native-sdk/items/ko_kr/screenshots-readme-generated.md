...
- `FastCommentsLiveCommenting` - 스레드 댓글, 투표, 답글, 페이지네이션, 멘션, 알림 및 실시간 업데이트를 지원합니다.
- `FastCommentsLiveChat` - 동일 엔진을 기반으로 한 채팅 프리셋: 최신 메시지가 아래에 위치하는 연대순 메시지, 리스트 아래에 컴포저, 실시간 헤더 스트립(연결 점 + 사용자 수), 스크롤 업으로 무한히 로드되는 히스토리, 새 메시지 자동 스크롤, 투표 및 답글 스레딩 없음. 모든 프리셋은 `config`를 통해 재정의할 수 있습니다.
...
- `renderDateBelowComment` - 댓글 아래에 날짜를 렌더링합니다.
- `showLiveStatus` - 댓글 위에 채팅 스타일의 "Live" + 사용자 수 헤더 스트립을 표시합니다.
- `useInlineSubmitButton` - 컴포저 안에 아이콘 형태의 제출 버튼을 렌더링합니다.
- `countAboveToggle` - `useShowCommentsToggle`와 함께, "Show Comments" 토글 위에 몇 개의 댓글을 렌더링할지 지정합니다.
- `preserveFeedScrollPosition` - `FastCommentsFeed`는 마운트 해제/재마운트 시 스크롤 오프셋을 기억합니다(기본값 true).
