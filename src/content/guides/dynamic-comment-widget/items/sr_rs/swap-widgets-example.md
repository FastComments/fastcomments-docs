Ово се све ради конфигурисањем [urlId](/guide-customizations-and-configuration.html#url-id) и повезаних параметара и позивањем `update()` на инстанцама widget-а како бисте их заменили другом темом коментара.

Испод је интерактиван пример (притисните run у горњем левом углу) који вам омогућава да по потреби додавате и замењујете теме коментара. Такође их можемо уклонити позивањем `instance.destroy` уместо update.

[inline-code-attrs-start title = 'Dodaj i zameni teme komentara'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // OVDE STAVITE VAŠ TENANT ID
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // за обавештења путем е-поште и контролну таблу за модерацију
                    "url": productURL, // за обавештења путем е-поште и контролну таблу за модерацију
                    "urlId": productID,
...
Ово је слично начину на који ради пагинација.

Код React-а и сродних библиотека можемо једноставно ажурирати конфигурацију, а библиотека (React, Angular, Vue) ће се побринути да освеже widget уместо нас. Код VanillaJS морамо то сами управљати, као што је приказано.

Слободно поставите било каква питања испод и даћемо све од себе да помогнемо!