Ово се ради подешавањем [urlId](/guide-customizations-and-configuration.html#url-id) и повезаних параметара и
позивом `update()` на инстанцама видгета да би их заменили другом темом.

Испод је интерактиван пример (притисните Run у горњем левом углу) који вам омогућава да по потреби додајете и мењате теме коментара.
Такође их можемо уклонити позивом `instance.destroy` уместо `update()`.

[inline-code-attrs-start title = 'Додај и замени теме коментара'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // СТАВИТЕ ОВДЈЕ ВАШ tenantId
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // за обавештења е-поштом и контролну таблу модерације
                    "url": productURL, // за обавештења е-поштом и контролну таблу модерације
                    "urlId": productID,
...
Ово је слично функционисању пагинације.

Код React-а и сродних библиотека можемо једноставно ажурирати конфигурацију, а библиотека (React, Angular, Vue)
ће се побринути за освежавање видгета за нас. Са VanillaJS-ом морамо то сами управљати, као што је показано.

Слободно поставите питања у наставку и учинићемо све да помогнемо!