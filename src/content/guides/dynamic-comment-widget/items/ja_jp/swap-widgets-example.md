すべては [urlId](/guide-customizations-and-configuration.html#url-id) と関連パラメータを設定し、ウィジェットインスタンスで `update()` を呼び出して別のスレッドに差し替えることで行われます。

下は対話式の例です（左上の「Run」を押してください）。これにより、必要に応じてコメントスレッドを追加・切り替えることができます。
また、update の代わりに `instance.destroy` を呼び出すことで削除することもできます。

[inline-code-attrs-start title = 'コメントスレッドの追加と切り替え'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // ここにあなたのテナントIDを入れてください
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // 通知メールとモデレーションダッシュボード用
                    "url": productURL, // 通知メールとモデレーションダッシュボード用
                    "urlId": productID,
...
これはページネーションの動作に似ています。

React や関連ライブラリを使用する場合は、設定を更新するだけでライブラリ（React、Angular、Vue）がウィジェットの再描画を処理してくれます。VanillaJS では、ここに示したように自分で管理する必要があります。

下に何か質問があればお気軽にどうぞ。できる限りお手伝いします！