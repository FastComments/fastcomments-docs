Dies wird alles durchgeführt, indem die [urlId](/guide-customizations-and-configuration.html#url-id) und zugehörige Parameter konfiguriert und
`update()` auf den Widget-Instanzen aufgerufen werden, um sie durch einen anderen Thread zu ersetzen.

Das Folgende ist ein interaktives Beispiel (oben links auf "Run" klicken), das es ermöglicht, Kommentar-Threads nach Bedarf hinzuzufügen und auszutauschen.
Wir könnten sie auch entfernen, indem wir statt `update` `instance.destroy` aufrufen.

[inline-code-attrs-start title = 'Kommentare hinzufügen und Threads austauschen'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // TRAGE HIER DEINE TENANT-ID EIN
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // für Benachrichtigungs-E-Mails und das Moderations-Dashboard
                    "url": productURL, // für Benachrichtigungs-E-Mails und das Moderations-Dashboard
                    "urlId": productID,
