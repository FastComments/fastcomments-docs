Tout cela se réalise en configurant le [urlId](/guide-customizations-and-configuration.html#url-id) et les paramètres associés et en appelant `update()` sur les instances du widget pour les remplacer par un fil différent.

L'exemple ci‑dessous est interactif (cliquez sur Exécuter en haut à gauche) et vous permet d'ajouter et d'échanger des fils de commentaires à la demande.
Nous pourrions également les supprimer en appelant `instance.destroy` au lieu de update.

[inline-code-attrs-start title = 'Ajouter et échanger des fils de commentaires'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // INSÉREZ VOTRE TENANT ID ICI
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // pour les courriels de notification et le tableau de bord de modération
                    "url": productURL, // pour les courriels de notification et le tableau de bord de modération
                    "urlId": productID,
...
Ceci est similaire au fonctionnement de la pagination.

Avec React et les bibliothèques associées, nous pouvons simplement mettre à jour la configuration et la bibliothèque (React, Angular, Vue) se chargera d'actualiser le widget pour nous. Avec VanillaJS, nous devons le gérer nous‑mêmes comme montré.

N'hésitez pas à poser des questions ci‑dessous et nous ferons de notre mieux pour vous aider !
