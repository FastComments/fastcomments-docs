Это всё делается путём настройки [urlId](/guide-customizations-and-configuration.html#url-id) и связанных параметров и вызова `update()` у экземпляров виджетов, чтобы заменить их другим потоком.

Ниже приведён интерактивный пример (нажмите Run в верхнем левом углу), который позволяет добавлять и переключать потоки комментариев по требованию. Мы также можем удалить их, вызвав `instance.destroy` вместо `update`.

[inline-code-attrs-start title = 'Добавление и переключение потоков комментариев'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // ПОДСТАВЬТЕ СЮДА ВАШ tenantId
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // для писем уведомлений и панели модерации
                    "url": productURL, // для писем уведомлений и панели модерации
                    "urlId": productID,
...
С React и сопутствующими библиотеками мы можем просто обновить конфигурацию, и библиотека (React, Angular, Vue) сама позаботится об обновлении виджета. С VanillaJS нам придётся управлять этим самостоятельно, как показано выше.

Не стесняйтесь задавать вопросы ниже — мы сделаем всё возможное, чтобы помочь!