Ово се све ради конфигурисањем [urlId](/guide-customizations-and-configuration.html#url-id) и повезаних параметара и позивањем `update()` на инстанцама видгета како бисте их замијенили другом нити.

Испод је интерактивни примјер (притисните 'Run' у горњем лијевом углу) који вам омогућава да по потреби додате и замјењујете коментарске нити.
Такођер их можемо уклонити позивом `instance.destroy` уместо `update`.

[inline-code-attrs-start title = 'Додавање и замјена коментарских нити'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // УПИШИТЕ ВАШ TENANT ID ОВДЈЕ
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // за е-поруке обавијештења и модерацијску контролну плочу
                    "url": productURL, // за е-поруке обавијештења и модерацијску контролну плочу
                    "urlId": productID,
...
Ово је слично томе како пагинација функционише.

Код React-а и сродних библиотека можемо једноставно ажурирати конфигурацију и библиотека (React, Angular, Vue) ће обрадити освјежење видгета за нас. Са VanillaJS-ом морамо сами управљати тим како је приказано.

Слободно поставите сва питања испод и учинићемо све да помогнемо!