Sve se to radi konfiguriranjem [urlId](/guide-customizations-and-configuration.html#url-id) i povezanih parametara te
pozivom `update()` na instancama widgeta kako bi ih zamijenili drugom niti.

Ispod je interaktivan primjer (pritisnite Run u gornjem lijevom kutu) koji vam omogućuje dodavanje i zamjenu niti komentara prema potrebi.
Također ih možemo ukloniti pozivom `instance.destroy` umjesto update.

[inline-code-attrs-start title = 'Dodaj i zamijeni niti komentara'; type = 'html'; isFunctional = true; inline-code-attrs-end]
[inline-code-start]
    ...
    <script>
        (function () {
            ...
            const tenantId = "demo"; // STAVITE OVDJE SVOJ TENANT ID
            ...
            function addWidget({name, productID, productURL}) {
                ...
                window.FastCommentsUI(widgetTarget, {
                    "tenantId": tenantId,
                    "pageTitle": name, // za obavijesti putem e-pošte i nadzornu ploču za moderiranje
                    "url": productURL, // za obavijesti putem e-pošte i nadzornu ploču za moderiranje
                    "urlId": productID,
...
Ovo je slično načinu na koji radi paginacija.

S Reactom i srodnim bibliotekama možemo jednostavno ažurirati konfiguraciju i biblioteka (React, Angular, Vue)
će se pobrinuti za osvježavanje widgeta. S VanillaJS-om moramo to sami upravljati kao što je prikazano.

Slobodno postavite pitanja u nastavku i učinit ćemo sve što možemo da pomognemo!