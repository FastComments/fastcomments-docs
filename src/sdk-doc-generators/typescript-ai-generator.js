...
/**
 * Documentation generator that creates API reference from TypeScript SDK
 * using AI to generate code examples
 */
class TypeScriptAIGenerator extends BaseDocGenerator {
    /**
     * Generate API reference documentation using AI
     * @returns {Promise<GeneratedDocs>}
     */
    async generate() {
        const config = this.sdk.typescriptAiConfig;
        if (!config) {
            throw new Error(`No typescriptAiConfig found for ${this.sdk.id}`);
        }
        ...
        for (const apiFile of config.apiFiles) {
            ...
            for (const method of methods) {
                ...
                if (!opInfo) {
                    // Try with capitalized first letter (e.g., addDomainConfig -> AddDomainConfig)
    ...
    generateMethodSection(method, codeExample, resource) {
        ...
        if (codeExample) {
            ...
            lines.push(`[inline-code-attrs-start title = '${title}'; type = 'typescript'; isFunctional = false; inline-code-attrs-end]`);
