...
const CppParser = require('./cpp-parser');
const OpenAIClient = require('./openai-client');

/**
 * Documentation generator that creates API reference from C++ SDK
 * using AI to generate code examples
 */
class CppAIGenerator extends BaseDocGenerator {
    /**
     * Generate API reference documentation using AI
     * @returns {Promise<GeneratedDocs>}
     */
    async generate() {
        const config = this.sdk.cppAiConfig;
        if (!config) {
            throw new Error(`No cppAiConfig found for ${this.sdk.id}`);
        }
    ...
    generateMethodSection(method, codeExample, resource) {
        ...
        if (codeExample) {
            ...
            lines.push(`[inline-code-attrs-start title = '${title}'; type = 'cpp'; isFunctional = false; inline-code-attrs-end]`);
